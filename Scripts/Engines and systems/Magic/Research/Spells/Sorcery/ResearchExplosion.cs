...
namespace Server.Spells.Research
{
	public class ResearchExplosion : ResearchSpell
	{
			...
			else if ( SpellHelper.CheckTown( p, Caster ) && CheckSequence() )
			{
				...
				if ( targets.Count > 0 )
				{
					...
					for ( int i = 0; i < targets.Count; ++i )
					{
						...
						Effects.SendLocationEffect( blast1, m.Map, Utility.RandomList( 0x36BD, 0x3822 ), 85, 10, Server.Items.CharacterDatabase.GetMySpellHue( Caster, 0 ), 0 );
		...
		private class InternalTarget : Target
		{
			...
			public InternalTarget( ResearchExplosion owner ) : base( Core.ML ? 12 : 14, true, TargetFlags.None )
			{
				m_Owner = owner;
			}
