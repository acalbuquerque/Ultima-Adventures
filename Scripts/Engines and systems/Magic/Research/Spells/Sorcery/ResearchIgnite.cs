...
namespace Server.Spells.Research
{
	public class ResearchIgnite : ResearchSpell
	{
			...
			else if ( SpellHelper.CheckTown( p, Caster ) && CheckSequence() )
			{
				...
				if ( targets.Count > 0 )
				{
					...
					for ( int i = 0; i < targets.Count; ++i )
					{
						...
						if ( Utility.RandomBool() ){ Effects.SendLocationEffect( blast2, m.Map, 0x3728, 85, 10, Server.Items.CharacterDatabase.GetMySpellHue( Caster, 0xB70 ), 0 ); }
						if ( Utility.RandomBool() ){ Effects.SendLocationEffect( blast3, m.Map, 0x3728, 85, 10, Server.Items.CharacterDatabase.GetMySpellHue( Caster, 0xB70 ), 0 ); }
						if ( Utility.RandomBool() ){ Effects.SendLocationEffect( blast4, m.Map, 0x3728, 85, 10, Server.Items.CharacterDatabase.GetMySpellHue( Caster, 0xB70 ), 0 ); }
						if ( Utility.RandomBool() ){ Effects.SendLocationEffect( blast5, m.Map, 0x3728, 85, 10, Server.Items.CharacterDatabase.GetMySpellHue( Caster, 0xB70 ), 0 ); }
		...
		private class InternalTarget : Target
		{
			...
			public InternalTarget( ResearchIgnite owner ) : base( Core.ML ? 12 : 14, true, TargetFlags.None )
			{
				m_Owner = owner;
			}
