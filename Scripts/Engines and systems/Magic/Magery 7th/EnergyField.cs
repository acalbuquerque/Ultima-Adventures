...
namespace Server.Spells.Seventh
{
	public class EnergyFieldSpell : MagerySpell
	{
		...
		public void Target( IPoint3D p )
		{
			...
			else if ( SpellHelper.CheckTown( p, Caster ) && CheckSequence() )
			{
				...
				for ( int i = -2; i <= 2; ++i )
				{
					...
					Effects.SendLocationParticles( EffectItem.Create( loc, Caster.Map, EffectItem.DefaultDuration ), 0x376A, 9, 10, Server.Items.CharacterDatabase.GetMySpellHue( Caster, 0 ), 0, 5051, 0 );
				}
			}
		...
		public class InternalTarget : Target
		{
			...
			public InternalTarget( EnergyFieldSpell owner ) : base( Core.ML ? 12 : 14, true, TargetFlags.None )
			{
				m_Owner = owner;
			}
