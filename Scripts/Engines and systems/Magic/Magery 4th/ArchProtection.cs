...
namespace Server.Spells.Fourth
{
	public class ArchProtectionSpell : MagerySpell
	{
					...
					if ( targets.Count > 0 )
					{
						for ( int i = 0; i < targets.Count; ++i )
						{
							...
							if ( m.BeginAction( typeof( ArchProtectionSpell ) ) )
							{
								...
								m.FixedParticles( 0x375A, 9, 20, 5027, Server.Items.CharacterDatabase.GetMySpellHue( Caster, 0 ), 0, EffectLayer.Waist );
								m.PlaySound( 0x1F7 );
							}
						}
					}
		...
		private class InternalTarget : Target
		{
			...
			public InternalTarget( ArchProtectionSpell owner ) : base( Core.ML ? 12 : 14, true, TargetFlags.None )
			{
				m_Owner = owner;
			}
